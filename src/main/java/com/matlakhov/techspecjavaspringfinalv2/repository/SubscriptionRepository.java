...
public interface SubscriptionRepository extends JpaRepository<SubscriptionEntity, Long> {
    ...
    /**
     * Находит все активные подписки пользователя по его идентификатору.
     *
     * @param userId идентификатор пользователя
     * @return список активных подписок, принадлежащих пользователю
     */
    @Query("SELECT s FROM SubscriptionEntity s WHERE s.userEntity.id = :userId AND s.isDeleted = false")
    List<SubscriptionEntity> findByUserEntityId(Long userId);

    /**
     * Находит топ подписок по популярности среди активных подписок.
    ...
    @Query("""
            SELECT s.serviceName, COUNT(s) 
             ...
             WHERE s.serviceName IS NOT NULL AND s.serviceName <> '' AND s.isDeleted = false 
    ...
    /**
     * Проверяет, существует ли активная подписка для пользователя с указанным названием сервиса.
     *
     * @param userId      идентификатор пользователя
     * @param serviceName название сервиса
     * @return true, если активная подписка существует, иначе false
     */
